...
class Order:
   def __init__(self, orderID: int, user: User, orderDate: datetime):
       ...
       self.__payment = None                         # Composition: Payment is part of Order
   ...
   # Getter and setter for payment (composition)
...
# Part 3: Ticket subclasses, RaceEvent class, and all remaining relationships with detailed comments
...
# -------------------------
# Subclasses: Ticket Variants
# -------------------------
class GroupTicket(Ticket):
   def __init__(self, ticketID: int, price: float, eventID: int, groupSize: int):
       super().__init__(ticketID, price, eventID)
       self.groupSize = groupSize


   def getGroupDiscount(self) -> float:
       return self.getPrice() * 0.10 if self.groupSize >= 5 else 0.0
...
class WeekendPackage(Ticket):
   def __init__(self, ticketID: int, price: float, eventID: int, includesCamping: bool):
       super().__init__(ticketID, price, eventID)
       self.includesCamping = includesCamping


   def getPerks(self) -> List[str]:
       return ["VIP Lounge", "Free Drinks"] if self.includesCamping else ["Free Drinks"]
...
class SingleRaceEvent(Ticket):
   ...
   def getRaceInfo(self) -> str:
       return f"Single entry to race: {self.raceName}"
